...
def _build_strategy_table(strategies):
    if not strategies:
        return html.Div(
            [
    ...
    columns = [
        {"name": "名称", "id": "name"},
        {"name": "类型", "id": "type"},
        {"name": "标的数", "id": "symbol_count"},
        {"name": "参数", "id": "params_summary"},
        {"name": "启用", "id": "enabled"},
    ...
    for s in strategies:
        ...
        params = s.get("params", {})
        if params:
            params_summary = ", ".join(f"{k}:{v}" for k, v in params.items())[:60]
        else:
            params_summary = "-"
        ...
        data.append(
            {
                ...
                "type_raw": s.get("type", ""),
                "symbol_count": sym_display,
                "params_summary": params_summary,
                "enabled": s.get("enabled", True),
                "created_at": s.get("created_at", "")[:16],
                "actions": f"edit:{s.get('name','')}|delete:{s.get('name','')}|export:{s.get('name','')}",
            }
        )

    return dash_table.DataTable(
        id="strategy-data-table",
        columns=columns,
        data=data,
        page_size=15,
