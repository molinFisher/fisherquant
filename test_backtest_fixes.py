"""量化系统改进清单逐条验证（对应 16 项改进）。

运行：.venv/Scripts/python.exe test_backtest_fixes.py
"""
import asyncio
import sys
...
def test_nav_single_ledger():
    ...
    check("P0-1 NAV 单一账本", correct and not buggy,
          f"nav={nav:.2f} avail+posmv={avail+pos_mv:.2f}")
    ...
    if buy_trades:
        ...
        for t in buy_trades:
            if t["trade_date"] == first_date:
                ...
                break
    check("P0-2 成交晚于信号(无同根向前看)", ok, detail or "ok")
...
def test_slippage():
    ...
    if not buy:
        check("P0-3 滑点", False, "无成交")
        return
    fill = buy[0]["price"]
    # 滑点后买入价应高于收盘价（10.0）
    ok = fill > 10.0
    expected = 10.0 * (1 + 10.0 / 10000.0)
    check("P0-3 滑点(买价抬高)", ok and abs(fill - expected) < 0.05,
          f"fill={fill:.4f} expected~{expected:.4f}")
...
def test_t1_unit():
    ...
    try:
        ...
        sold_ok = False
    check("P0-4 T+1 当日买入被冻结、次日解冻后可卖",
          avail_after_buy == 0 and avail_after_settle == 100 and sold_ok,
...
def test_risk_reject():
    ...
    check("P0-5 风险引擎拒单生效", rejected and pos_val <= 100.0 + 1.0,
...
def test_conditions():
    ...
    check("P0-6 止损条件单触发成交", sold, f"pos={paper.get_positions().get('000001.SZ')}")
...
def test_short():
    ...
    avail = paper.get_account()["available"]
    check("P1-8 做空支持(负持仓)", short_ok and avail > 1_000_000.0,
...
def test_limit_lock():
    ...
    check("P2-16 涨停不可买入", len(filled1) == 0 and len(filled2) == 0 and len(filled3) == 0,
          f"filled={len(filled1)+len(filled2)+len(filled3)}")


def main():
    test_nav_single_ledger()
    test_no_lookahead()
    test_slippage()
    test_t1_unit()
    test_risk_reject()
